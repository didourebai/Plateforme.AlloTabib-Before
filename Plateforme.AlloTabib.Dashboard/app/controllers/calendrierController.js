...
app.controller('calendrierController', function ($scope, $location, $window, calendrierApiServices, $modal) {
    $.unblockUI();
   ...
   // init();
            ...
            calendrierApiServices.creneauAyantRdv(praticien, dateSelect, heure.heureDebutCalendrier).then(
               // Case : Success.
                        function (results) {
                            ...
                            if (results.data.data.praticienCin == null || results.data.data.statut == 'C' || results.data.data.statut == 'R')
                            {
                                //ici c'est un créneau ajouter par le praticien
                                $scope.praticienCreneaux = true;
                            }
                            else
                            {
    ...
    $scope.confirmerOuRejeterCreneau = function (cinpatient, heuredeb, statutc,patientcin) {
        ...
        calendrierApiServices.updateRendezVous(rendezVous).then(
            ...
            function (results) {
                $.unblockUI();
                toastr.success("Vous avez mis à jour le rendez vous avec succès.");
