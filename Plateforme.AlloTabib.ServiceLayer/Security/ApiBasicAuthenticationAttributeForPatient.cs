...
{
    public class ApiBasicAuthenticationAttributeForPatient : AuthorizeAttribute
    {
                ...
                if (auth.Scheme == "Basic" && !string.IsNullOrEmpty(auth.Parameter))
                {
                    ...
                    if (gotIt)
                    {
                        ...
                        if (alloTabibUserServices != null)
                        {
                            ...
                            var isAuthenticated = alloTabibUserServices.IsPatientAuthenticated(userName, password);
